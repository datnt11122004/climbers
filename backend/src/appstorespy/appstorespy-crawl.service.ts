...
export class AppStoreSpyCrawlService {
    ...
    async executeCrawl() {
        ...
        try {
            ...
            this.logger.log(
                `Fetching active apps from database for daily crawl...`
            );
            ...
            this.logger.log(
                `✅ Crawl finished: ${processedCount}/${totalAppsFound} apps processed`
            );
    ...
    async crawlAppData(
        ...
        if (appRecord.store === 'PLAY') {
            ...
            const dailyData =
                await this.appStoreSpyService.getPlayDailyInstalls(
                    appRecord.appId,
                    startDate,
                    endDate
                );
            ...
            for (const entry of dailyData) {
                ...
                await this.prisma.appDailyData.upsert({
                    ...
                    create: {
                        ...
                        updatedDate: searchDataRef?.updated
                            ? new Date(searchDataRef.updated)
                            : null,
                        rawResponse: entry as any
                    }
                });
