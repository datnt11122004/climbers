...
export class AppStoreSpyTriggerService {
	...
	private async evaluateAppTriggers(trackedAppId: number): Promise<number> {
		...
		if (
			appAgeDate >= thirtyDaysAgo &&
			d0.downloads >= 20_000
		) {
			const growthRate = d2.downloads > 0 ? d0.downloads / d2.downloads : d0.downloads;
			...
			if (!existing) {
				const daysSinceRelease = Math.floor((today.getTime() - appAgeDate.getTime()) / (1000 * 60 * 60 * 24));
				...
				alertsCreated++;
				this.logger.log(`🆕 NT3 Alert for app #${trackedAppId}: ${daysSinceRelease} ngày tuổi, D0=${d0.downloads}`);
