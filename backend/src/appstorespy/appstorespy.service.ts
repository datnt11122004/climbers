...
export class AppStoreSpyService {
    ...
    async getPlayApp(bundleId: string, country = 'US') {
        try {
            ...
            this.logger.error(
                `Failed to fetch Play app: ${bundleId}`,
                error?.message
            );
            return null;
        }
    ...
    async getPlayDailyInstalls(bundleId: string, start?: string, end?: string) {
        try {
            const { data } = await this.client.get(
                `/play/apps/${bundleId}/installs_daily`,
                {
                    params: { start, end }
                }
            );
            ...
            this.logger.error(
                `Failed to fetch Play daily installs: ${bundleId}`,
                error?.message
            );
            return [];
        }
    ...
    async getPlayEstimates(bundleId: string, start?: string, end?: string) {
        try {
            ...
            this.logger.error(
                `Failed to fetch Play estimates: ${bundleId}`,
                error?.message
            );
            return [];
        }
    ...
    async searchPlayApps(
        ...
        try {
            ...
            console.log(error);
            this.logger.error(
                `Failed to search Play apps: ${query}`,
                error?.message
            );
            return [];
        }
    ...
    async queryPlayApps(page = 1, limit = 100) {
        try {
            ...
            console.log(error);
            this.logger.error(
                `Failed to query Play apps (page ${page})`,
                error?.message
            );
            return [];
        }
