...
export class AuthenticationBotService {
    ...
    authentication(botId: number): Middleware<Context> {
        return async (ctx: Context, next): Promise<void> => {
            if (!ctx.session)
                throw new Error(
                    'Session middleware must be applied before authentication middleware'
                );
            ...
            if (!ctx.from || !ctx.chat) {
                await ctx.reply(
                    'Authentication failed. Cannot get user information.'
                );
                return;
            }
            ...
            try {
                ...
                if (ctx.chat.type === 'private') {
                    ...
                    this.logger.debug(
                        `User ${telegramId} authenticated via bot ${botId}`
                    );
                }
